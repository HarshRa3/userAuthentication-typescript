...
export const GET = async (request:NextRequest) => {
  ...
  try {
    ...
    return ResponseMessages("Registerd User getting successfully",registeredUserForCourses,true,200);
  } catch (error) {
...
export const POST = async (request:NextRequest) => {
    const { username, email, course, userId, dateOfBirth, description } =await request.json();
    try {
    ...
    await registeredUserForCourse.create({ username, email, course, userId, dateOfBirth, description });
